...
class Pipe:
    ...
    def _content_to_text(content) -> str:
        """Coerce OpenAI-style message content to plain text.

        `content` is normally a string, but when an image is attached to a
        message it arrives as a multipart list instead:
            [{"type": "text", "text": "..."}, {"type": "image_url", ...}]
        Calling string methods (.strip(), .lower(), etc.) directly on that
        list is what produces "'list' object has no attribute 'strip'".
    ...
    def classify_intent(self, messages: List[Dict]) -> str:
        last_msg = self._content_to_text(messages[-1].get("content")).lower()
        ...
        recent = messages[-6:]
        formatted = "\n".join(
            f"{m.get('role', '').upper()}: {self._content_to_text(m.get('content'))}"
            for m in recent
        )
...
Reply with ONLY one word: mcq, thinking, or default."""
    ...
    def _build_payload_messages(
        ...
        for offset, msg in enumerate(trimmed):
            ...
            # user_messages entries are already plain text by this point
            # (see the "Clean messages" step in pipe()), but coerce again
            # defensively in case this is ever called with raw messages.
    ...
    def _extract_content(self, choice: dict) -> str:
        ...
        if "delta" in choice and isinstance(choice["delta"], dict):
            return self._content_to_text(choice["delta"].get("content", ""))
        if "message" in choice and isinstance(choice["message"], dict):
            return self._content_to_text(choice["message"].get("content", ""))
        return ""
